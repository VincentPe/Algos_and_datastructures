...
class HierarchiesShortestPath:
    ...
    def __init__(self, data, settings):
        ...
        self.x, self.y = data[2:self.n * 2 + 2][::2], data[2:self.n * 2 + 2][1::2]
        data, self.edges = self.get_edges(data)
        data, self.adj, self.cost = self.get_adjacents_and_costs(data)
    ...
    def get_edges(self, data):
        data = data[self.n * 2 + 2:]
        return data, list(zip(zip(data[0:(3 * self.m):3], data[1:(3 * self.m):3]), data[2:(3 * self.m):3]))

    def get_adjacents_and_costs(self, data):
        data = data[self.m * 3:]
    ...
    def clean_variables(self, s, t):
        ...
        self.potential = self.initiate_potentials()
    ...
    def process_edge(self, v, side):
        for u_idx, u in enumerate(self.adj[side][v]):
            if self.node_order[u] > self.node_order[v]:
                ...
                new_potential = self.approx_potential(u, side)

                if self.potential[side][u] > (self.dist[side][v] + self.cost[side][v][u_idx] + new_potential):
                    self.potential[side][u] = self.dist[side][v] + self.cost[side][v][u_idx] + new_potential
    ...
    def approx_potential(self, u, side):
        dist_f = self.euclidean_distance(self.x[u], self.x[self.endpoints[0]], self.y[u], self.y[self.endpoints[0]])
        dist_b = self.euclidean_distance(self.x[u], self.x[self.endpoints[1]], self.y[u], self.y[self.endpoints[1]])
...
# node order: [3, 10, 4, 1, 5, 7, 9, 8, 6, 2]
...
if __name__ == '__main__':
    ...
    for q in range(navi.q):
        navi.clean_variables(navi.s[q], navi.t[q])
